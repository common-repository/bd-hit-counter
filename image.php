<?php
      /* 
      Plugin Name: Bliss Drive Hit Counter Plugin
      Plugin URI: http://www.blissdrive.com/bliss-hit-counter/
    ...
    */
      ...
      class HitCounter extends HookdResource {
          ...
          function display() {
              ...
              echo '<div class="bd-hit-counter"'.$alignment_options.'>';
              //if (get_option('wphcu_pad_zeros') && strlen($hits) < 7) {
                  for ($i = 0; $i < (4 - strlen($hits)); $i++) {
                      echo "<img src='".WP_PLUGIN_URL."/bd-hit-counter/designs/$style/0.gif'>";
                  }
             // }                       
              echo preg_replace('/(\d)/', "<img src='".WP_PLUGIN_URL."/bd-hit-counter/designs/$style/$1.gif'>", $hits);
			  if (get_option('wphcu_display_credit') == 1) { 
			  	echo '<br /><small>Hit Counter provided by <a href="http://www.blissdrive.com/">Orange County Web Design</a></small>';
			  } else {
			  	echo '<!-- Hit Counter provided by Orange County Web Design http://www.blissdrive.com/-->;
			  }
              echo '</div>';
          }
      }
      ...
      class wHitCounter extends WP_Widget {
          ...
          function form($instance) {
              echo 'Please go to <a href="options-general.php?page=bd-hit-counter">Settings -> Bliss Hit Counter</a> to configure this sidebar widget';
          }
